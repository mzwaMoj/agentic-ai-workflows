...
class Text2SQLEngine:
    ...
    async def process_query(self, user_input: str, chat_history: Optional[List[Dict]] = None) -> Dict[str, Any]:
        ...
        # Start MLflow tracking (following chatbot.py)
    ...
    async def _handle_sql_analysis(self, user_input: str, function_args: dict, 
        ...
        try:
            ...
            chart_keywords = ["chart", "graph", "plot", "visual", "pie", "bar", "barh", "line", "scatter"]
    ...
    def _agent_table_retriever(self, user_request: str, agent_table_router):
        ...
        try:
            ...
            if tool_call.function.name == "agent_table_rag":
                ...
                # Use the vector service's query engine directly like chatbot.py
                try:
                    table_results = self.vector_service.query_engine.query(
                        "retrieve the full tables metadata without intepreting or editing anything for the following given tables: " + 
                        str(args.get("relevant_tables"))
                    )
